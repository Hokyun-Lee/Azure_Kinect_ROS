...
#undef LIST_ENTRY
  ...
  if (params_.body_tracking_enabled) {
    ...
    joint_vector_publisher_ = node_.advertise<std_msgs::Float64MultiArray>("Avatar/joint_vector_data", 100);
            ...
            else
            {
              if (body_marker_publisher_.getNumSubscribers() > 0)
              {
                ...
                for (size_t i = 0; i < num_bodies; ++i)
                {
                  ...
                  k4a_float3_t Wrist_left = body.skeleton.joints[7].position;
                  std::cout << "[Wrist_left position] "<< std::endl;
                  std::cout << "(x, y, z) = (" << Wrist_left.v[0] << ", " << Wrist_left.v[1] << ", " << Wrist_left.v[2] << ")" << std::endl;
                  k4a_float3_t Wrist_right = body.skeleton.joints[14].position;
                  std::cout << "[Wrist_right position] "<< std::endl;
                  std::cout << "(x, y, z) = (" << Wrist_right.v[0] << ", " << Wrist_right.v[1] << ", " << Wrist_right.v[2] << ")" << std::endl; 
                  k4a_float3_t Head_position = body.skeleton.joints[27].position;
                  std::cout << "[Head position] "<< std::endl;
                  std::cout << "(x, y, z) = (" << Head_position.v[0] << ", " << Head_position.v[1] << ", " << Head_position.v[2] << ")" << std::endl << std::endl;
                  
                  float HeadToWrist_left_x = Wrist_left.v[0] - Head_position.v[0];
                  float HeadToWrist_left_y = Wrist_left.v[1] - Head_position.v[1];
                  float HeadToWrist_left_z = Wrist_left.v[2] - Head_position.v[2];
                  std::cout << "[Head to Wrist_left Vector] "<< std::endl;
                  std::cout << "(x, y, z) = (" << HeadToWrist_left_x << ", " << HeadToWrist_left_y << ", " << HeadToWrist_left_z << ")" << std::endl << std::endl;

                  std_msgs::Float64MultiArray array;
		              //Clear array
		              array.data.clear();
		              //for loop, pushing data in the size of the array
			            array.data.push_back(HeadToWrist_left_x);
                  array.data.push_back(HeadToWrist_left_y);
                  array.data.push_back(HeadToWrist_left_z);
                  joint_vector_publisher_.publish(array);
